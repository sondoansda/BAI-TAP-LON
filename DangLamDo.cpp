...
void ThemVao( QL arr[],int &n,char TenFile[]){
	...
	f=fopen(TenFile,"ab");
     	n++;
     	printf("\nTen nguoi tiem thu %d : ",n);fflush(stdin);gets(arr[n-1].Ten);
		printf("CCCD : ");fflush(stdin);gets(arr[n-1].CCCD);ChuanHoaSo(arr[n-1].CCCD);
        printf("Que quan : ");fflush(stdin);gets(arr[n-1].QueQuan);
        printf("So dien thoai : ");fflush(stdin);gets(arr[n-1].SDT);
		int c;
		printf("\nSo mui da tiem : ");scanf("%d",&c);
		arr[n-1].id= c;
        for(int j = 0;j<c;j++){
        printf("\nThong tim mui %d : ",j+1);
        printf("\nTen mui tiem : ");fflush(stdin);gets(arr[n-1].a[j].TenMui);
        printf("Quoc gia : ");fflush(stdin);gets(arr[n-1].a[j].QuocGia);
       do{
       	printf("Loai : ");scanf("%d",&arr[n-1].a[j].Loai);
       }while(arr[n-1].a[j].Loai<100);
        do{
        printf("Khoang cach tiem : ");scanf("%d",&arr[n-1].a[j].kcTiem);
...
if(n==0){
  ...
  fclose(f);                   
 while(1){  
 printf("\n---------------------------CHUONG TRINH QUAN LY NGUOI TIEM------------------------------");
 printf("\n1.Hien thi thong tin nguoi tiem ");
 printf("\n2.Them nguoi tiem ");
 printf("\n3.Cap nhap TT nguoi tiem");
 printf("\n4.Xoa nguoi tiem khoi danh sach");
 printf("\n5.Thoat");int b;
 do{
 	printf("\nLua chon cua ban : ");scanf("%d",&b);
 }while(b<1 && b>4);
 switch (b){                        
